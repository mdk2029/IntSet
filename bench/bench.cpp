...
class UnorderedSetFixture : public ::benchmark::Fixture {
public:
    UnorderedSetFixture() : ::benchmark::Fixture(),
        gen_(time(nullptr)),
        dis_{0,std::numeric_limits<int64_t>::max()}
    {
    }
...
BENCHMARK_DEFINE_F(UnorderedSetFixture, Lookup)(benchmark::State& state) {
    const int size = static_cast<int>(state.range(0));
    for (auto _ : state) {
        for (int i = 0; i < size; ++i) {
            benchmark::DoNotOptimize(data_.find(dis_(gen_)));
...
BENCHMARK_REGISTER_F(UnorderedSetFixture, Lookup)->RangeMultiplier(2)->Range(1000000, 32000000);
